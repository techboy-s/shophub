...
// UPDATE: Changed 'title' to 'name' to match your database schema
interface Product {
  id: number;
  name: string; 
  price: number; 
...
export default function AddToCartButton({ product }: { product: Product }) {
  ...
  // Added 'e' event parameter to prevent bubbling if used inside cards
  const handleAddToCart = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault(); // Prevents clicking the product link if this button is inside a card
    
    // UPDATE: Use 'product.name' directly
    addItem({
      id: product.id,
      name: product.name,
      price: product.price,
