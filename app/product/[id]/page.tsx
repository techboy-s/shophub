...
import { notFound } from "next/navigation";
import { Metadata } from "next";
import ProductDetails from "@/components/ProductDetails"; // Import the component we just made
...
// 2. SERVER PAGE COMPONENT (Fetches Data)
export default async function ProductPage({ params }: PageProps) {
  const { id } = await params;
  
  // Direct Database Call (Only allowed in Server Components)
  const product = await prisma.product.findUnique({
    where: { id: Number(id) },
  });
  ...
  // We convert price to number just in case Decimal types cause issues
  return <ProductDetails product={{ ...product, price: Number(product.price) }} />;
}